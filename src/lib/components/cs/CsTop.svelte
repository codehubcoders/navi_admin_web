<script>
    ...
    let selectePeriod= "전체";
    let periodRadio = ["전체", "상세설정"];

    function onChangePeriod(event) {
        selectePeriod = event.currentTarget.value;
    }
    let selectedDate = 1;
...
<div class="radio-group">
    ...
    <div class="radio-box">
        ...
        <div class="radio">
            {#each periodRadio as item, index}
            <label style={selectePeriod != item ? "color:#757779" : ""}>
                <input
                    checked={selectePeriod == item}
